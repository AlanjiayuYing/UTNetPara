...
LR_Reduce_No_Test_Improvement = 15
EARLY_STOP_LR_TOO_LOW = 1e-5
...
class MyLRScheduler():
    ...
    # when utilizing a test data set as recommended/commonly suggested
    def stepTrainTest(self, newTestScore, logger):
        # Update learning rate
        if newTestScore <= self.bestValue:
            self.noImprovement += 1
            logger.info('###Now no: ' + str(self.noImprovement))

            if self.noImprovement >= LR_Reduce_No_Test_Improvement:
                self.model.load_state_dict(torch.load(self.foldResultsModelPath + '/currentBestValModel.pt'))
                self.update_lr_by_divison(self.divideLRfactor)
                logger.info('### After ' + str(LR_Reduce_No_Test_Improvement) + ' no val score improvement => Best model loaded and LR reduced to ' + str(self.currentLR) + ' !')
                if self.currentLR < EARLY_STOP_LR_TOO_LOW:
                    return True
                self.noImprovement = 0
        else:
            self.noImprovement = 0
            self.bestValue = newTestScore
            torch.save(self.model.state_dict(), self.foldResultsModelPath + '/currentBestValModel.pt')
    ...
    '''
    # loads current model with highest validation accuracy into current model
    def loadBestTestIntoModel(self):
        self.model.load_state_dict(torch.load(self.foldResultsModelPath + '/currentBestValModel.pt'))
