...
from model import Custom, UTNet, UTNetPara, UTNetParaV2  #add new model UTNet
...
testDatasetsSizes = np.array([160, 60, 60, 60, 60, 60])     #change size
#testDatasetsSizes = np.array([32, 12, 12, 12, 12, 12])     #change size
...
def train(model, setting, optimizer, scheduler, epochs, batchSize, logger, resultsPath, testResults, testResultsTTA, tbWriter, allClassEvaluators):
    ...
    for epoch in range(epochs):
        ...
        for batch in dataloader_Train:
            ...
            #diceLoss = Dice_Loss_ut(prediction, segBatch)

            #loss = 0.3 * CEloss + 0.7 * diceLoss
        ...
        if epoch % 50 == 0:
            with torch.no_grad():
                tbWriter.add_image('Train/_img', torch.round((imgBatch[0,:3,:,:] + 1.6) / 3.2 * 255.0).byte() , epoch)
        ...
        if 'val' in setting:
            ...
            if epoch % 50 == 0:
                with torch.no_grad():
                    tbWriter.add_image('Val/_img', torch.round((imgBatch[0,:3,:,:] + 1.6) / 3.2 * 255.0).byte(), epoch)
        ...
        if 'test' in setting:
            ...
            if epoch % 50 == 0:
                with torch.no_grad():
                    tbWriter.add_image('Test/_img', torch.round((imgBatch[0,:3,:,:] + 1.6) / 3.2 * 255.0).byte(), epoch)
            ...
            # scheduler.step()
            if 'test' in setting:
                endLoop = scheduler.stepTrainTest(test_mean_score, logger)
            else:
                endLoop = scheduler.stepTrain(epochTrainLoss, logger)

            if epoch == (
                    epochs - 1):  # when no early stop is performed, load bestValModel into current model for later save
                logger.info('### No early stop performed! Best val model loaded... ####')
                if 'test' in setting:
                    scheduler.loadBestTestIntoModel()
                    ...
                    for sampleNo in test_idx:
                        ...
                        if applyTestTimeAugmentation:
                            ...
                            if saveFinalTestResults:
                                ...
                                npPath = npFolder + '/test_idx_' + str(sampleNo) + '.npy'
                                saveFigureResults(imgBatchCPU, outputPrediction, postprocessedPrediction, finalPredictionRGB, segBatch.squeeze(0).numpy(), preprocessedGT, preprocessedGTrgb, fullResultPath=figPath, alpha=0.4)
                                np.save(npPath, postprocessedPrediction)
...
def set_up_training(modelString, setting, epochs, batchSize, lrate, weightDecay, logger, resultsPath):
    ...
    elif modelString == 'utnet':
        #According to RGB and set of unet, set input_channels as 3
        model = UTNet(3, options.base_chan, options.num_class, reduce_size=options.reduce_size,
                      ...
                      maxpool=True)
    elif modelString == 'utnetpara':
        model = UTNetPara(3, options.base_chan, options.num_class, reduce_size=options.reduce_size,
                      block_list=options.block_list, num_blocks=options.num_blocks, num_heads=[4, 4, 4, 4],
                      projection='interp', attn_drop=0., proj_drop=0., rel_pos=True, aux_loss=options.aux_loss,
                      maxpool=True)
    elif modelString == 'utnetparav2':
        model = UTNetParaV2(3, options.base_chan, options.num_class, reduce_size=options.reduce_size,
                      block_list=options.block_list, num_blocks=options.num_blocks, num_heads=[4, 4, 4, 4],
                      projection='interp', attn_drop=0.1, proj_drop=0.1, rel_pos=True, aux_loss=options.aux_loss,
                      maxpool=True)
    ...
    #optimizer = optim.SGD(model.parameters(), lr=lrate, momentum=0.9, weight_decay=weightDecay)

    #scheduler = MyLRScheduler(optimizer, model, resultsModelPath, setting, initLR=lrate, divideLRfactor=3.0)
    scheduler = MyLRScheduler(optimizer, model, resultsModelPath, setting, initLR=lrate, divideLRfactor=2.0)
...
if '__main__' == __name__:
    ...
    parser = argparse.ArgumentParser(description='python training.py -m <model-type> -d <dataset> -s <train_valid_test>' +
                                                 '--reduce_size <reduce_size> --block_list <block_list>'+
                                                 '--aux_loss <aux_loss> -e <epochs>'+
                                                 '-b <batch-size> -r <learning-rate> -w <weight-decay>')
    ...
    # Parameters from UTNet


    parser.add_argument('--reduce_size', dest='reduce_size', default=8, type=int)
    parser.add_argument('--block_list', dest='block_list', default='1234', type=str)
    #Since use add_argument, num_block cannot be changed with initial paramter setting up
    parser.add_argument('--num_blocks', dest='num_blocks', default=[1, 3, 6, 1], type=str)
    #the number of output channels for eight class segmentation
    #parser.add_argument('--num_class', type=int, dest='num_class', default=4, help='number of segmentation classes')
    parser.add_argument('--num_class', type=int, dest='num_class', default=8, help='number of segmentation classes')
    parser.add_argument('--base_chan', type=int, dest='base_chan', default=32,
                      help='number of channels of first expansion in UNet')
    parser.add_argument('--aux_loss', dest='aux_loss', action='store_true', help='using aux loss for deep supervision')

    options = parser.parse_args()
    #assert(options.model in ['custom', 'unet_mod', 'ce_mod'])
    assert (options.model in ['custom', 'utnet', 'unet_mod', 'ce_mod', 'utnetpara', 'utnetparav2'])   #add model utnet
    ...
    #resultsPath = '/home/students/ying/jupyter_code/Unet_test/UTNet_Test_result_local'
    resultsPath = "/work/scratch/ying/TestResult"
